...
public class Detach_Square2 extends PApplet { // Category (0-3) adj Category (8-9),     8 not adj 9 
	...
	private void optimize( ) {  
		try {
			...
			model.addConstr(typepr, GRB.LESS_EQUAL, 4, "");// sum of type 4-7 <= threshold
			...
			for (int k = 8; k < 10; k++)
				for (int i = 0; i < P.length; i++)
					typepr.addTerm(1, X[k][i]);
			model.addConstr(typepr, GRB.GREATER_EQUAL, 12, ""); // sum of type 8-9 >= threshold
