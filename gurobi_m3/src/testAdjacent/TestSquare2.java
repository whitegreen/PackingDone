...
public class TestSquare2 extends PApplet {  //same results by Python: IP Adj / Test_square2.py
	private int[][] P; //  grid
	...
	public void setup() {
		...
		int[][] mm = new int[K][];
		for (int k = 0; k < K; k++)
			mm[k] = new int[] { (M.has(AH_is, k) ? 2 : 1), 7 }; // {1,4} works
		mm[8] = new int[] { 2, 7 };
	...
	println();
		...
		int[][] pairs = { { 0, 8 }, { 1, 8 }, { 2, 8 }, { 3, 8 }, { 8, 0 }, { 8, 1 }, { 8, 2 }, { 8, 3 } };
